...
$html1->load($result4);
$html1=$html1->find('div[class=portletBody]',0);
if(strpos($login, 'Home')){
  try{

array_push($loginArr,array('login'=>'true',
  'heading'=>trim($html1->find('h3',0)),
  'instructions'=>trim($html1->find('div[class=textPanel]',0))
));
}catch(\Exception $e){
  array_push($loginArr,array('login'=>'error'));
}
